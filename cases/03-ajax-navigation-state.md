...
**요구** — 트래픽이 가장 많은 페이지에서 리로드 없이 필터링과 목록을 갱신
**난점** — 페이지 로드가 알아서 해주던 것들이 전부 직접 관리해야 할 상태가 됨

**어려웠던 지점**

- **페이지 로드는 모든 걸 리셋하고, 그게 구조를 떠받치고 있었습니다.** 필터 패널이 열렸는지 닫혔는지, 어떤 facet이 적용됐는지, 스크롤 위치는 어디인지, 어떤 카드가 렌더됐는지 — 리로드하면 URL에서 다시 만들어지니 저절로 맞았습니다. 리로드를 부분 갱신으로 바꾸는 순간 이 전부가 어긋날 수 있는 상태가 됩니다.
- **뒤로 가기에는 고객의 기대가 붙어 있습니다.** 필터를 세 개 걸고 뒤로 가기를 누른 고객은 필터 하나가 풀리기를 기대하지 페이지를 떠나기를 기대하지 않습니다. 필터를 바꿀 때마다 히스토리를 쌓지 않으면 뒤로 가기가 목록을 통째로 벗어나고, 고객 눈에는 사이트가 자기 작업을 날린 걸로 보입니다.
- **딥링크로 같은 상태가 재현돼야 합니다.** 클릭으로는 도달하는데 URL을 붙여넣으면 도달 못 하는 필터 목록은 공유에서도, 검색엔진에서도, 북마크에서도 깨진 겁니다.
- **부분 갱신은 낡은 DOM을 남깁니다.** 목록만 갈아끼우고 거기 의존하던 것들을 그대로 두면, 이전 상태의 요소가 화면에 남고 옛 핸들러에도 여전히 묶여 있습니다.
- **필터 의미론이 자명하지 않습니다.** facet 안에서의 다중 선택, facet끼리의 상호작용, "전체 해제"가 어디까지 해제하는지, 성별 토글이 필터인지 컨텍스트 전환인지. 하나하나가 구현 전에 확정돼야 하는 제품 결정이고, 각각이 상태에 담을 것을 바꿉니다.

**접근**

- **URL이 곧 상태이고 나머지는 전부 거기서 파생됩니다.** 적용된 facet은 쿼리 파라미터에 있습니다. 목록은 URL을 보고 렌더되며, 그 URL이 내비게이션으로 왔든 필터 클릭으로 왔든 뒤로 가기로 왔든 붙여넣기로 왔든 똑같습니다. 이 결정 덕분에 딥링크와 히스토리가 따로 만들 기능이 아니라 자연스러운 결과가 됩니다.
- **필터를 바꾸면 히스토리가 쌓입니다.** 그래서 뒤로 가기가 페이지를 떠나지 않고 필터 상태를 하나씩 되짚습니다.
- **뷰 상태와 필터 상태를 분리했습니다.** 필터 패널이 열려 있는지는 필터가 아닙니다. 둘을 합치면 UI 상태가 URL에 들어가고, 그게 공유되고 북마크돼서 기본값이 달라야 할 다른 뷰포트로 돌아옵니다. 패널 상태는 페이지 내 이동을 넘어 유지되지만 공유되는 URL에는 들어가지 않습니다.
- **성별 선택은 facet이 아니라 컨텍스트 전환으로 정의했습니다.** 지금 집합을 좁히는 게 아니라, 어떤 상품 집합과 어떤 카테고리 집합을 볼지 자체를 바꾸기 때문입니다. 이게 정리되고 나서 필터 코드의 분기 하나가 아니라 자체 정책을 가진 별도 모듈로 뺐습니다. 덕분에 [케이스 01](01-breaking-up-shared-files.md)에서 줄이려던 공유 파일 바깥에 두는 효과도 있었습니다.
- **갱신 후 렌더링은 컴포넌트마다 한 곳이 맡습니다.** 상품 카드, 위시리스트 표시, 연관 섹션이 각각 단일 진입점을 통해 다시 렌더됩니다. 기능마다 변화를 눈치챈 자리에서 DOM을 패치하지 않습니다. 낡은 요소 버그는 부분 갱신자가 여럿이라 생기고, 해법은 렌더 대상마다 하나만 두는 것입니다.
- **상태별로 눈으로 검증했습니다.** 필터 열림과 닫힘, 다중 선택, 전체 해제, 다른 목록에서 들어온 경우, 딥링크로 들어온 경우. 조합마다 캡처해서 비교했습니다. 필터 상태 버그는 조합적이라 코드 리뷰에서는 거의 안 보입니다. 상태를 직접 만들어놓고 봐야 잡힙니다.
